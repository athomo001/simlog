...
def show_splash_screen():
    splash_root = tk.Tk()
    splash_root.title("Inicio")
    splash_root.configure(bg='white')  # Fondo blanco

    # --- Configuraciones de la ventana ---
    window_width = 450
    window_height = 250
    splash_root.overrideredirect(True)  # Quita bordes y botones de la ventana
    ...
    # --- Contenido ---
    # Usar un Canvas para dibujar los bordes
    canvas = tk.Canvas(splash_root, width=window_width, height=window_height, bg='white', highlightthickness=0)
    canvas.pack()

    # Dibujar bordes
    pad = 20  # Espacio desde el borde de la ventana
    border_width_outer = 2
    border_width_inner = 4  # Grosor del borde rojo

    # Borde exterior negro
    canvas.create_rectangle(
        pad, pad,
        window_width - pad, window_height - pad,
        outline='black', width=border_width_outer
    )
    # Borde interior rojo
    inner_pad = pad + border_width_outer + 5  # Espacio entre bordes
    canvas.create_rectangle(
        inner_pad, inner_pad,
        window_width - inner_pad, window_height - inner_pad,
        outline='red', width=border_width_inner
    )
    # Borde interior (opcional, para el recuadro del texto)
    text_pad = inner_pad + border_width_inner + 5
    canvas.create_rectangle(
        text_pad, text_pad,
        window_width - text_pad, window_height - text_pad,
        outline='black', width=1  # Borde fino
    )

    # --- Texto ---
    # Fuente grande para SIMlog
    font_title = tkFont.Font(family="Helvetica", size=36, weight="bold")
    canvas.create_text(window_width / 2, window_height / 2 - 20, text="SIMlog", font=font_title, fill="black", anchor="center")

    # Fuente más pequeña para el subtítulo
    font_subtitle = tkFont.Font(family="Helvetica", size=12)
    canvas.create_text(window_width / 2, window_height / 2 + 40, text="Simulador de log multi marca", font=font_subtitle, fill="black", anchor="center")

    # --- Duración y cierre ---
    splash_root.update()  # Asegura que la ventana se dibuje
    time.sleep(3)  # Muestra la pantalla por 3 segundos
...
if __name__ == '__main__':
    # Mostrar la pantalla de inicio antes de cualquier otra acción
    print("Mostrando pantalla de inicio...")
    show_splash_screen()
    print("Pantalla de inicio cerrada. Iniciando script principal...")
